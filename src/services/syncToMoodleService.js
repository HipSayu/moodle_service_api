...
class SyncToMoodleService {
      ...
      if (midtermSection) {
        ...
        if (!hasMidtermQuiz) {
          ...
          try {
            ...
            syncLogger.info(
              `✓ Created midterm quiz for course ${courseId}: ${
                midtermQuiz.quizId || "unknown"
              }`
            );
          } catch (quizError) {
            syncLogger.error(
      ...
      if (finalSection) {
        ...
        if (!hasFinalAssignment) {
          ...
          try {
            ...
            syncLogger.info(
              `✓ Created final assignment for course ${courseId}: ${
                finalAssignment.assignmentid || "unknown"
              }`
            );
        ...
        try {
          await retryOperation(
            async () => {
              ...
              let courseData = {};
              if (course.IDToBoMon) {
                const category = await moodleService.getCategoryByIdNumber(
                  course.IDToBoMon.toString()
                );
                if (!category) {
                  console.log(
                    `Category not found for department ID ${course.IDToBoMon}`
                  );
                  courseData = {
              ...
              if (existingCourse) {
                ...
                try {
                  ...
                  syncLogger.error(
                    `Failed to ensure sections for existing course ${existingCourse.id}:`,
                    sectionError
                  );
                }
              } else {
                ...
                try {
                  ...
                  if (course.SoTietThucHanh < course.SoTietLyThuyet) {
                    ...
                    sections = [
                      ...
                      {
                        name: "Chương 1",
                        summary: "Nội dung chương 1",
                        visible: 1,
                        position: 0,
                      },
                      {
                        name: "Chương 2",
                        summary: "Nội dung chương 2",
                        visible: 1,
                        position: 0,
                      },
                      {
                        name: "Chương 3",
                        summary: "Nội dung chương 3",
                        visible: 1,
                        position: 0,
                      },
                      {
                    ...
                    sections = [
                      ...
                      {
                        name: "Buổi Thông 1",
                        summary: "Buổi Thông 1",
                        visible: 1,
                        position: 0,
                      },
                      {
                        name: "Buổi Thông 2",
                        summary: "Buổi Thông 2",
                        visible: 1,
                        position: 0,
                      },
                      {
                  ...
                  for (const sectionData of sections) {
                    const section = await moodleService.createSectionWithPlugin(
                      newCourse.id,
                      sectionData
                    );
                  ...
                  syncLogger.error(
                    `Failed to create content for course ${newCourse.id}:`,
                    contentError
                  );
                  console.log(
                    `Lỗi tạo nội dung cho khóa học ${courseData.course_fullname}`
                  );
                }
              }
              syncCount++;
          ...
          await retryOperation(
            async () => {
              ...
              if (result && result.success && result.quizId) {
                ...
                throw new Error(
                  `Plugin returned unsuccessful result for course ${
                    course.fullname
  ...
  async syncAssignmentGrades() {
    try {
      ...
      syncLogger.info(
        `Found ${gradesFromDB.length} grade records from database${
        ...
        try {
          await retryOperation(
            async () => {
              ...
              syncLogger.info(
                `✓ Processed course ${course.fullname}: ${
